        ...
        for (int i = 0; i < drones.Count; i++)
        {
            ...
            switch (d.State)
            {
                case DroneWorker.DroneState.Idle:
                    ...
                    slot.SetProgressColor(idleProgressColor);   // ★ Inspector の色
                    break;
                ...
                case DroneWorker.DroneState.MovingToTarget:
                    {
                        ...
                        slot.SetProgressColor(movingProgressColor);   // ★ Inspector
                        break;
                    }
                ...
                case DroneWorker.DroneState.ReturningToBase:
                    {
                        ...
                        slot.SetProgressColor(returningProgressColor); // ★ Inspector
                        break;
                    }
                ...
                case DroneWorker.DroneState.Working:
                    {
                        ...
                        if (task != null)
                        {
                            if (task.kind == DroneBuildManager.TaskKind.MineResource)
                            {
                                ...
                                label = !string.IsNullOrEmpty(itemName) ? $"採掘中：{itemName}" : "採掘中…";

                                slot.SetProgressColor(miningProgressColor);  // ★ Inspector
                            }
                            else
                            {
                                // 建築
                                if (task.def != null)
                                    label = $"建築中：{task.def.displayName}";

                                slot.SetProgressColor(buildingProgressColor); // ★ Inspector
                            }
                        }
    ...
    void UpdateDetailPanel()
    {
        ...
        if (detailSubText != null)
        {
            ...
            else
            {
                ...
                // ★ ここから分岐：Jobごとに表示内容を変える
                string extra = "";

                if (item.boundDrone.job == DroneWorker.JobType.Miner)
                {
                    // 従来どおり：採掘ログ
                    string miningSummary = item.boundDrone.GetMinedItemSummary();
                    extra = miningSummary;
                }
                else if (item.boundDrone.job == DroneWorker.JobType.Builder)
                {
                    // Builder：所持素材（_buildCargoItems）の一覧
                    string held = item.boundDrone.GetHeldItemSummary();
                    extra = held;
                }
